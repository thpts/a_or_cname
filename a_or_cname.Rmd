...
# system calls here because they are faster and easier for me
total_nx <- as.numeric(system("ggrep -v ',IN,' data/top1m_cnames.csv | wc -l", intern=TRUE))
total_sf <- as.numeric(system("ggrep ',,$'     data/top1m_cnames.csv | wc -l", intern=TRUE))
totals <- system("grep ',IN' data/top1m_cnames.csv | awk -F, '{print $5}' | sort | uniq -c", intern=TRUE)

for(type in totals){ 
  type_split <- strsplit(trimws(type), " ")
  value   <- c(value, type_split[[1]][1])
  percent <- c(percent, as.numeric(type_split[[1]][1]) / 10000)
  record  <- c(record, type_split[[1]][2])
}

# NXDOMAIN values
value   <- c(value, as.numeric(total_nx))
percent <- c(percent, as.numeric(total_nx) / 10000)
record  <- c(record, "NXDOMAIN")

# SERVFAIL values
value   <- c(value, as.numeric(total_sf))
percent <- c(percent, as.numeric(total_sf) / 10000)
