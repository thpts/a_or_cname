...
con <- dbConnect(RSQLite::SQLite(), paste0(dataDir, "journal-cyber-policy-2019/", "2018-08-16-snapshot.sqlite"))
...
```
-- For each domain, list each distinct ASN in use
SELECT DISTINCT domain, asn FROM record
...
SELECT COUNT(DISTINCT asn) FROM record
WHERE record_type IN("A", "AAAA")
AND response_code = 0
AND is_www = 0
AND parent IS NOT NULL
